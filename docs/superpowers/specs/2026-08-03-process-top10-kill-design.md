...
| Q2 | 范围 | 所有**用户进程**上榜;root 进程因非特权 app 读取 EPERM 原理性不可见(2026-08-03 最终审查实测确认),「系统」角标+置灰代码保留作防御但不可达;活动监视器靠特权才能看 root 进程 |
...
- root 进程显示:非特权 app 读不到 root 进程 CPU/内存(EPERM,实测),接受该限制不做 sysctl 兜底(兜底也排不进 Top 10,价值为零)。
